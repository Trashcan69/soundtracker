    ...
    GdkInputCondition condition)
{
    ...
    while (!lockok) {
        ...
        case DSERR_BUFFERLOST:
            if
                FAILED(IDirectSoundBuffer_Restore(d->wbuffer))
            lockok = 2;
            break;
        default:
            lockok = 2;
...
dsound_play(void* dp)
{
    ...
    if
        FAILED(IDirectSoundBuffer_SetFormat(d->pbuffer, &d->pbformat))
        {
            if (d->writeprim) {
    ...
    if (d->writeprim) {
        ...
        if
            FAILED(IDirectSound_CreateSoundBuffer(d->lpds, &d->sbdesc, &d->sbuffer, 0))
        return FALSE;
    ...
    if
        FAILED(IDirectSoundBuffer_Lock(d->wbuffer, 0, 0, &d->lockbuf, &d->locklen, 0, 0, DSBLOCK_ENTIREBUFFER))
        {
    ...
    if
        FAILED(IDirectSoundBuffer_Play(d->wbuffer, 0, 0, DSBPLAY_LOOPING))
        {
            if (d->writeprim) {
                d->writeprim = 0;
                IDirectSound_SetCooperativeLevel(d->lpds, d->winhwnd, DSSCL_PRIORITY);
...
dsound_open(void* dp)
{
    ...
    if
        FAILED(d->DSCreate(0, &d->lpds, 0))
    return FALSE;

    d->dscaps.dwSize = sizeof(DSCAPS);
    if
        FAILED(IDirectSound_GetCaps(d->lpds, &d->dscaps))
    return FALSE;

    if
        FAILED(IDirectSound_SetCooperativeLevel(d->lpds, d->winhwnd, d->writeprim ? DSSCL_WRITEPRIMARY : DSSCL_PRIORITY))
        {
            d->writeprim = 0;
            if
                FAILED(IDirectSound_SetCooperativeLevel(d->lpds, d->winhwnd, DSSCL_PRIORITY))
            return FALSE;
        }
    ...
    if
        FAILED(IDirectSound_CreateSoundBuffer(d->lpds, &d->pbdesc, &d->pbuffer, 0))
    return FALSE;
