 ...
 *
 * Copyright (C) 2006, 2022 Yury Aliaev
...
update_freqs_list(alsa_driver* d, gboolean set_highest)
{
    ...
    for (i = 0; i < NUM_FREQS; i++) {
        if ((mixfreqs[i] >= minfreq) && (mixfreqs[i] < maxfreq)) {
...
update_estimate(alsa_driver* d)
{
    ...
    buf = g_strdup_printf(_("%f milliseconds"), 1000 * (double)periodsize / (double)d->playrate);
...
update_controls(alsa_driver* d)
{
    ...
    set_highest_possible(d->prefs_resolution_w, ARRAY_SIZE(d->prefs_resolution_w));
    gtk_widget_set_sensitive(d->prefs_channels_w[0], d->canmono);
    gtk_widget_set_sensitive(d->prefs_channels_w[1], d->canstereo);
    set_highest_possible(d->prefs_channels_w, ARRAY_SIZE(d->prefs_channels_w));
...
pcm_open_and_load_hwparams(alsa_driver* d, gboolean force_nonblock)
{
    ...
    if ((err = snd_pcm_open(&(d->soundfd), d->device,
             ...
             force_nonblock ? SND_PCM_NONBLOCK : 0))
        < 0) {
...
check_period_sizes(alsa_driver* d)
{
    ...
    if ((err = snd_pcm_hw_params_set_format(d->soundfd, d->hwparams,
             (d->bits - 8) ? FORMAT_16 : d->signedness8 ? SND_PCM_FORMAT_S8 : SND_PCM_FORMAT_U8))
        < 0) {
...
set_rates(alsa_driver* d, guint channels, guint format)
{
    ...
    if ((err = snd_pcm_hw_params_get_rate_min(d->hwparams, &ratemin, 0)) < 0) {
        alsa_error(N_("Unable to get minimal sampling rate"), err);
        return -1;
    }
    d->minfreq[format] = MAX(ratemin, 8000);

    if ((err = snd_pcm_hw_params_get_rate_max(d->hwparams, &ratemax, 0)) < 0) {
        alsa_error(N_("Unable to get maximal sampling rate"), err);
        return -1;
    }
...
device_test(GtkWidget* w, alsa_driver* d)
{
    ...
    new_device = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(d->alsa_device));

    if (g_ascii_strcasecmp(d->device, new_device)) {
        g_free(d->device);
        d->device = g_strdup(new_device);
        gui_combo_box_prepend_text_or_set_active(GTK_COMBO_BOX_TEXT(d->alsa_device), d->device, FALSE);
    }
    ...
    if (d->can8) {
        if ((err = snd_pcm_hw_params_set_format(d->soundfd, d->hwparams,
                 d->signedness8 ? SND_PCM_FORMAT_S8 : SND_PCM_FORMAT_U8))
            < 0) {
            alsa_error(N_("Unable to set audio format"), err);
            snd_pcm_close(d->soundfd);
            return;
    ...
    if (d->can16) {
        ...
        if (pcm_open_and_load_hwparams(d, TRUE) < 0)
            return;
        if ((err = snd_pcm_hw_params_set_format(d->soundfd, d->hwparams,
                 d->signedness16 ? SND_PCM_FORMAT_S16 : SND_PCM_FORMAT_U16))
            < 0) {
            alsa_error(N_("Unable to set audio format"), err);
            snd_pcm_close(d->soundfd);
            return;
    ...
    if (!d->can16 && !d->can8) {
        error_error(_("Neither 8 nor 16 bit resolution is supported by ALSA device!"));
        snd_pcm_close(d->soundfd);
        return;
    }
...
prefs_channels_changed(GtkWidget* w, alsa_driver* d)
{
    ...
    if ((curr = find_current_toggle(d->prefs_channels_w,
        ARRAY_SIZE(d->prefs_channels_w))) < 0)
        return;
    d->stereo = curr;
...
prefs_resolution_changed(GtkWidget* w, alsa_driver* d)
{
    ...
    if ((curr = find_current_toggle(d->prefs_resolution_w,
             ARRAY_SIZE(d->prefs_resolution_w))) < 0)
        return;
    d->bits = (curr + 1) * 8;
...
prefs_bufsize_changed(GtkWidget* w, alsa_driver* d)
{
    ...
    expression = g_strdup_printf(_(" = %u discretes"), 1 << d->buffer_size);
...
prefs_periods_changed(GtkWidget* w, alsa_driver* d)
{
    ...
    expression = g_strdup_printf(" = %u", 1 << d->num_periods);
...
{
    d->hwtest = FALSE;
    gui_combo_box_prepend_text_or_set_active(GTK_COMBO_BOX_TEXT(d->alsa_device), d->device, TRUE);
    update_controls_a(d);
    d->hwtest = TRUE;
}
...
devices_row_selected(alsa_driver* d)
{
    ...
    gui_combo_box_prepend_text_or_set_active(GTK_COMBO_BOX_TEXT(d->alsa_device), dev, TRUE);
    g_free(dev);
}
...
static void device_list(GtkWidget* w, alsa_driver* d)
{
    ...
    GtkTreeIter iter;
    GtkWidget* thing;
    const gchar* titles[] = { N_("Dev"), N_("Description") };
    ...
    while (card >= 0) {
        ...
        while (1) {
            ...
            g_snprintf(name, sizeof(name), "hw:%i,%i", card, dev);
    ...
    gtk_widget_show_all(d->devices_dialog);
}
...
alsa_make_config_widgets(alsa_driver* d)
{
    ...
    static const char* resolutionlabels[] = { N_("8 bit"), N_("16 bit") };
    ...
    d->alsa_device = gtk_combo_box_text_new_with_entry();
    ...
    if (d->playback) {
        ...
        thing = gtk_label_new(_("Estimated audio delay:"));
        gtk_box_pack_start(GTK_BOX(box2), thing, FALSE, FALSE, 0);
        gtk_widget_show(thing);

        d->estimatelabel = thing = gtk_label_new("");
        gtk_widget_set_tooltip_text(box2, _("The playback will start and stop immediately, "
...
alsa_playing (void* data)
{
    ...
    DRIVER_THREAD_LOOP_BEGIN(d, TRUE)
        ...
        towrite = d->p_fragsize;
        buffer = d->sndbuf;
        if (!d->firstpoll) {
            while (towrite > 0) {
                gint res;

                if (!d->processing) /* Driver is stopping; we should skip sound output */
                    break;
                ...
                if (d->verbose)
                    g_print("Written: %li from %li discretes\n", w, d->p_fragsize);
        ...
        d->callback(d->sndbuf, d->p_fragsize, d->p_mixfreq, d->mf);
...
static void*
alsa_sampling (void *data)
{
    alsa_driver * const d = data;
    snd_pcm_sframes_t toread;
    guint size;
    gint8 *buffer;

    DRIVER_THREAD_LOOP_BEGIN(d, FALSE)
        /* Since this parameters could be modified when the driver was waiting */
        toread = d->p_fragsize;
        size = d->stereo + (d->bits >> 4);
        buffer = d->sndbuf;
        ...
        while (toread > 0) {
            ...
            if (d->verbose)
                g_print("Read: %li from %li discretes\n", w, d->p_fragsize);
        ...
        if (d->processing) {
            ...
            if (d->callback(d->sndbuf, d->p_fragsize << size, d->p_mixfreq, d->mf)){
                DOUBLE_BUFFER_ALLOCATE(d, d->sndbuf, d->p_fragsize << size)
                if (!d->sndbuf) {
                    error_error(_("ALSA driver: Out of memory error!"));
                    DRIVER_THREAD_ERROR
                }
            } else {
...
static void*
alsa_new_out(gboolean (*callback)(void *buf, guint32 count, gint mixfreq, gint mixformat))
{
    return alsa_new(TRUE, callback);
}

static void*
alsa_new_in(gboolean (*callback)(void *buf, guint32 count, gint mixfreq, gint mixformat))
{
    return alsa_new(FALSE, callback);
}
...
alsa_open(void* dp)
{
    ...
    if (err < 0) {
        alsa_error(d->playback ? N_("Unable to determine current swparams for playback")
                               : N_("Unable to determine current swparams for capture"),
            err);
        goto out;
    }
    ...
    if (err < 0) {
        alsa_error(d->playback ? N_("Unable to set start threshold mode for playback")
                               : N_("Unable to set start threshold mode for capture"),
            err);
        goto out;
    }
    ...
    if (err < 0) {
        alsa_error(d->playback ? N_("Unable to set avail min for playback")
                               : N_("Unable to set avail min for capture"),
            err);
        goto out;
    }
    ...
    if (err < 0) {
        alsa_error(d->playback ? N_("Unable to set sw params for playback")
                               : N_("Unable to set sw params for capture"),
            err);
        goto out;
    }
    ...
    DOUBLE_BUFFER_INIT(d, d->sndbuf, !d->playback, d->p_fragsize << size)
    if (!d->sndbuf) {
        error_error(_("ALSA driver: Out of memory error!"));
        goto out;
    }
    ...
    const gchar* f)
{
    ...
    for (i = 0; i < size; i++)
        gui_combo_box_prepend_text_or_set_active(GTK_COMBO_BOX_TEXT(d->alsa_device), devlist[i], FALSE);
...
static void
alsa_activate (void* dp, const gchar* group)
{
    alsa_driver * const d = dp;

    DRIVER_THREAD_NEW(d, d->playback ? alsa_playing : alsa_sampling, dp,
        d->playback ? "Playback" : "Sampling")
}

static void
alsa_deactivate (void* dp)
{
    alsa_driver * const d = dp;
