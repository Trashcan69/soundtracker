...
/*
 * The Real SoundTracker - Driver support routines (header)
 *
 * Copyright (C) 2018, 2022 Yury Aliaev
...
#define DOUBLE_BUFFER_STUFF void *sndbuf0, *sndbuf1;\
    gboolean is_buffer1;

#define DOUBLE_BUFFER_ALLOCATE(dp, where, size)  if (dp->is_buffer1)\
        where = dp->sndbuf1 = malloc(size);\
    else\
        where = dp->sndbuf0 = malloc(size);

#define DOUBLE_BUFFER_SWOP(dp, where) if (dp->is_buffer1)\
        where = dp->sndbuf0;\
    else\
        where = dp->sndbuf1;\
    dp->is_buffer1 = !dp->is_buffer1;

#define DOUBLE_BUFFER_INIT(dp, where, cond, size) dp->sndbuf0 = malloc(size);\
    if (cond)\
        dp->sndbuf1 = malloc(size);\
    where = dp->sndbuf0;\
    dp->is_buffer1 = FALSE;

#define DOUBLE_BUFFER_PREINIT(dp) dp->sndbuf0 = dp->sndbuf1 = NULL;

#define DOUBLE_BUFFER_FREE(dp) if (dp->sndbuf0) {\
        free(dp->sndbuf0);\
        dp->sndbuf = NULL;\
    }\
    if (dp->sndbuf1) {\
